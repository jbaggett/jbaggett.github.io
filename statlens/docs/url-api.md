...
| `ci_method` | string | `percentile` | Which CI(s) the results show on **bootstrap** pages: `percentile` (default, IMS), `se` (estimate ± z·SE — the "±2 SE" 95% rule of thumb), or `both`. A live **Percentile \| ±2 SE \| Both** toggle is also in the results. | `?ci_method=both` |
