...
**Category:** Conceptual
**Description:** Rocks buried under a plot of ground, sorted by size with depth (pebbles on top, boulders at the bottom); the parameter is their average weight. Five designs draw from the same population — **simple random**, **stratified** (proportional allocation across the three depth layers), **cluster** (dig a few whole holes, weigh everything in them), **multistage** (a few holes, then a sample within each), and **convenience** (scrape the surface). Each sample reports x̄ against μ plus its *cost*: rocks weighed and holes dug. **Run 500 of each design** builds the sampling distribution of x̄ for all five on one shared axis, with a table of bias, SD, spread relative to simple random, and mean cost. An **expert-only cluster shape** control switches clusters between *deep holes* (each cutting all three layers) and *shallow pits* (each at a single depth) — the same design, the same n, and a six-fold change in spread (measured: 0.57× simple random's SD with deep holes, 3.48× with shallow pits). Pits are the usual real-world case, which is why textbooks say cluster sampling costs precision; holes are the favourable case. Hidden behind **More options** because it is a second question, after the four designs are straight — Todd Will's note that an earlier *horizontal layers* version blurred the cluster/stratum distinction. Adapted from Todd Will's Mathematica demo, with the rigged SRS removed — the original deletes middle-stratum points to make simple random *look* unbalanced.
...
| `clusters` | string | `holes` | `holes` or `pits` — the cluster geometry for the cluster and multistage designs. A pinned value also reveals the control, which is otherwise expert-only. | `?design=cluster&clusters=pits` |
