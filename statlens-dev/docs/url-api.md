...
| `data` | float[] | _(none)_ | Inline numeric data. Honoured on the **one-sample** pages only — `bootstrap-mean` (a list of measurements) and `bootstrap-prop` (a list of 0/1 outcomes). **Ignored** on the two-group and paired pages (`bootstrap-two-means`, `bootstrap-two-props`, `bootstrap-paired`, `randomization-diff-means`, `randomization-diff-props`, `randomization-paired`): a flat comma list cannot express two groups or matched pairs, so those pages skip it rather than load half a dataset. Also ignored when `dataset` is present. Implemented in `js/page-utils.js` (`initDataPanel`), not in `sim-app.js` — grepping the page engine for it will come up empty. | `?data=1.2,3.4,5.6` |
